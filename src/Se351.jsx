...
function Se351() {
    return (
        <article>
            ...
            <br/>
            <p>Storage Access - we aim to create the illusion of quickly accessible and permanent massive storage. We achieve 'quickly accessible' and 'massive' using Buffer manage, which manages the memory and move of data between the man memory and the disk. Achieve 'quickly accessible' and 'reliable' using RAID(Redundant Arrays of Independent Disks) a disk organisation technique which manages a large number of disks with high data capacity, access speed, reliability. Achieve 'non-volatile' using log disk the disk devoted to writing sequential log for block updates. <b>RAID</b> When using multiple disks, the chance of data loss is much higher. To avoid this we allow redundancy, storing extra information to rebuild the information incase of a disk failure. Techinique 1 - stripping improves transfer rate by stripping data across multiple disks, technique 2: mirroring, duplicates every disk -- costly. Technique 3 - parity, introduce the parity bit which allows error correction. (Block interleaved distributed parity)</p>

            <br/>
            <h3>Indexing</h3>
            <p>If we store all of the files in order, we can find records using binary search. However maybe another alternative is indexing... We can have sparse or dense indices. The index on the ordering field of the ordered file: primary index- the index on the ordering field which has a unique constraint, clustering index - the index on the ordering field, multiple records can have the same value. Secondary indices</p>
